...
const songs = ALPHABET.map(
    (letter, index) => {
        ...
        return {
            ...
            path:
                `${CONFIG.musicPath}${letter}${number}.mp4`,
            thumbnail:
                `${CONFIG.thumbnailsPath}${number}.png`,
            trackNumber: number
        };
    }
);
...
const photos = Array.from(
    ...
    (_, index) => {
        ...
        return {
            ...
            path:
                `${CONFIG.photosPath}${number}.${extension}`,
            number
        };
    }
);
...
async function syncSharedAnalytics() {
    ...
    try {
        ...
        console.warn(
            "Shared Firebase analytics unavailable; using local analytics.",
            error
        );
    }
}
 ...
 *
 * Important:
 * mediaElapsedSeconds = total visible time
 *
 * mediaCommittedSeconds = portion already
 * saved into analytics
 *
 * Isliye analytics har 5 seconds update
 * ho sakti hai without visible timer
 * ko 0 par reset kiye.
 */
...
function registerMediaOpen(
    ...
    if (
        ...
        state.currentSession.mediaViews[
            media.id
        ] =
            normalizeSeconds(
                state.currentSession.mediaViews[
...
function commitCurrentMediaTime(
    ...
    state.currentSession.mediaWatchTime[
        media.id
    ] =
        normalizeSeconds(
            state.currentSession.mediaWatchTime[
    ...
    /*
     * Timer display ko reset nahi karna.
     * Sirf batana hai ki kitna time
     * analytics mein already commit ho chuka hai.
     */
    state.mediaCommittedSeconds =
        totalSeconds;
